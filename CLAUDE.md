...
**Bots.** `server/bot.ts` defines **Kishore**, a server-driven bot — _not_ part of the engine. The host adds bots from the lobby (`ADD_BOT` wire message); each bot occupies a player seat with a generated id (`bot-…`) and a display name (default `"Kishore"`, auto-suffixed for collisions: `"Kishore 2"`, `"Kishore 3"`, …). The store persists bot ids on the game row as `botPlayerIds` so cold-start hydration and reconnects know which seats to drive. After every accepted event, `socket.ts` runs a per-game bot loop — `nextBotPlayerId` → `pickBotAction` → `manager.process` — with a small random delay between steps for UX. Strategy is hardcoded (always draw from deck, replace when drawn < highest unlocked, call showdown when hand value < 10, 30% mistake rate on slot selection); the bot reads its own hand via `engine.getState()` (the documented escape hatch).
